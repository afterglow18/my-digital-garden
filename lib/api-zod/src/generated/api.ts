/**
 * Generated by orval v8.18.0 🍺
 * Do not edit manually.
 * Api
 * Outfit Generator API
 * OpenAPI spec version: 0.1.0
 */
import * as zod from 'zod';


/**
 * Returns server health status
 * @summary Health check
 */
export const HealthCheckResponse = zod.object({
  "status": zod.string()
})


/**
 * @summary List all clothing items
 */
export const ListClothingQueryParams = zod.object({
  "category": zod.enum(['outfits', 'beauty', 'toiletries', 'essentials']).optional().describe('Filter by category')
})

export const ListClothingResponseItem = zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "category": zod.enum(['outfits', 'beauty', 'toiletries', 'essentials']),
  "imageObjectPath": zod.string().nullable(),
  "color": zod.string().nullish(),
  "brand": zod.string().nullish(),
  "size": zod.string().nullish(),
  "season": zod.string().nullish(),
  "occasion": zod.string().nullish(),
  "purchasePrice": zod.string().nullish(),
  "purchaseDate": zod.string().nullish(),
  "notes": zod.string().nullish(),
  "isFavorite": zod.boolean().optional(),
  "timesWorn": zod.number().optional(),
  "createdAt": zod.coerce.date()
})
export const ListClothingResponse = zod.array(ListClothingResponseItem)


/**
 * @summary Add a new clothing item
 */



export const CreateClothingItemBody = zod.object({
  "name": zod.string().min(1),
  "category": zod.enum(['outfits', 'beauty', 'toiletries', 'essentials']),
  "imageObjectPath": zod.string().optional(),
  "color": zod.string().optional(),
  "brand": zod.string().optional(),
  "size": zod.string().optional(),
  "season": zod.string().optional(),
  "occasion": zod.string().optional(),
  "purchasePrice": zod.string().optional(),
  "purchaseDate": zod.string().optional(),
  "notes": zod.string().optional(),
  "isFavorite": zod.boolean().optional()
})

export const CreateClothingItemResponse = zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "category": zod.enum(['outfits', 'beauty', 'toiletries', 'essentials']),
  "imageObjectPath": zod.string().nullable(),
  "color": zod.string().nullish(),
  "brand": zod.string().nullish(),
  "size": zod.string().nullish(),
  "season": zod.string().nullish(),
  "occasion": zod.string().nullish(),
  "purchasePrice": zod.string().nullish(),
  "purchaseDate": zod.string().nullish(),
  "notes": zod.string().nullish(),
  "isFavorite": zod.boolean().optional(),
  "timesWorn": zod.number().optional(),
  "createdAt": zod.coerce.date()
})


/**
 * @summary Get wardrobe statistics by category
 */
export const GetWardrobeStatsResponse = zod.object({
  "total": zod.number(),
  "byCategory": zod.array(zod.object({
  "category": zod.string(),
  "count": zod.number()
})),
  "favorites": zod.number(),
  "outfitsGenerated": zod.number()
})


/**
 * @summary Generate a random outfit from the wardrobe
 */
export const GenerateOutfitBody = zod.object({
  "excludeCategories": zod.array(zod.string()).optional()
})

export const GenerateOutfitResponse = zod.object({
  "items": zod.array(zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "category": zod.enum(['outfits', 'beauty', 'toiletries', 'essentials']),
  "imageObjectPath": zod.string().nullable(),
  "color": zod.string().nullish(),
  "brand": zod.string().nullish(),
  "size": zod.string().nullish(),
  "season": zod.string().nullish(),
  "occasion": zod.string().nullish(),
  "purchasePrice": zod.string().nullish(),
  "purchaseDate": zod.string().nullish(),
  "notes": zod.string().nullish(),
  "isFavorite": zod.boolean().optional(),
  "timesWorn": zod.number().optional(),
  "createdAt": zod.coerce.date()
}))
})


/**
 * @summary Get a clothing item by ID
 */
export const GetClothingItemParams = zod.object({
  "id": zod.coerce.number()
})

export const GetClothingItemResponse = zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "category": zod.enum(['outfits', 'beauty', 'toiletries', 'essentials']),
  "imageObjectPath": zod.string().nullable(),
  "color": zod.string().nullish(),
  "brand": zod.string().nullish(),
  "size": zod.string().nullish(),
  "season": zod.string().nullish(),
  "occasion": zod.string().nullish(),
  "purchasePrice": zod.string().nullish(),
  "purchaseDate": zod.string().nullish(),
  "notes": zod.string().nullish(),
  "isFavorite": zod.boolean().optional(),
  "timesWorn": zod.number().optional(),
  "createdAt": zod.coerce.date()
})


/**
 * @summary Update a clothing item
 */
export const UpdateClothingItemParams = zod.object({
  "id": zod.coerce.number()
})




export const UpdateClothingItemBody = zod.object({
  "name": zod.string().min(1).optional(),
  "category": zod.enum(['outfits', 'beauty', 'toiletries', 'essentials']).optional(),
  "imageObjectPath": zod.string().optional(),
  "color": zod.string().optional(),
  "brand": zod.string().optional(),
  "size": zod.string().optional(),
  "season": zod.string().optional(),
  "occasion": zod.string().optional(),
  "purchasePrice": zod.string().optional(),
  "purchaseDate": zod.string().optional(),
  "notes": zod.string().optional(),
  "isFavorite": zod.boolean().optional(),
  "timesWorn": zod.number().optional()
})

export const UpdateClothingItemResponse = zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "category": zod.enum(['outfits', 'beauty', 'toiletries', 'essentials']),
  "imageObjectPath": zod.string().nullable(),
  "color": zod.string().nullish(),
  "brand": zod.string().nullish(),
  "size": zod.string().nullish(),
  "season": zod.string().nullish(),
  "occasion": zod.string().nullish(),
  "purchasePrice": zod.string().nullish(),
  "purchaseDate": zod.string().nullish(),
  "notes": zod.string().nullish(),
  "isFavorite": zod.boolean().optional(),
  "timesWorn": zod.number().optional(),
  "createdAt": zod.coerce.date()
})


/**
 * @summary Delete a clothing item
 */
export const DeleteClothingItemParams = zod.object({
  "id": zod.coerce.number()
})

export const DeleteClothingItemResponse = zod.void()


/**
 * @summary List all saved outfits
 */
export const ListOutfitsResponseItem = zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "notes": zod.string().nullish(),
  "itemIds": zod.array(zod.number()),
  "items": zod.array(zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "category": zod.enum(['outfits', 'beauty', 'toiletries', 'essentials']),
  "imageObjectPath": zod.string().nullable(),
  "color": zod.string().nullish(),
  "brand": zod.string().nullish(),
  "size": zod.string().nullish(),
  "season": zod.string().nullish(),
  "occasion": zod.string().nullish(),
  "purchasePrice": zod.string().nullish(),
  "purchaseDate": zod.string().nullish(),
  "notes": zod.string().nullish(),
  "isFavorite": zod.boolean().optional(),
  "timesWorn": zod.number().optional(),
  "createdAt": zod.coerce.date()
})).optional(),
  "createdAt": zod.coerce.date()
})
export const ListOutfitsResponse = zod.array(ListOutfitsResponseItem)


/**
 * @summary Save a generated outfit
 */




export const SaveOutfitBody = zod.object({
  "name": zod.string().min(1),
  "notes": zod.string().optional(),
  "itemIds": zod.array(zod.number()).min(1)
})

export const SaveOutfitResponse = zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "notes": zod.string().nullish(),
  "itemIds": zod.array(zod.number()),
  "items": zod.array(zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "category": zod.enum(['outfits', 'beauty', 'toiletries', 'essentials']),
  "imageObjectPath": zod.string().nullable(),
  "color": zod.string().nullish(),
  "brand": zod.string().nullish(),
  "size": zod.string().nullish(),
  "season": zod.string().nullish(),
  "occasion": zod.string().nullish(),
  "purchasePrice": zod.string().nullish(),
  "purchaseDate": zod.string().nullish(),
  "notes": zod.string().nullish(),
  "isFavorite": zod.boolean().optional(),
  "timesWorn": zod.number().optional(),
  "createdAt": zod.coerce.date()
})).optional(),
  "createdAt": zod.coerce.date()
})


/**
 * @summary Add an item to a saved outfit
 */
export const AddItemToOutfitParams = zod.object({
  "id": zod.coerce.number()
})

export const AddItemToOutfitBody = zod.object({
  "itemId": zod.number()
})

export const AddItemToOutfitResponse = zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "notes": zod.string().nullish(),
  "itemIds": zod.array(zod.number()),
  "items": zod.array(zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "category": zod.enum(['outfits', 'beauty', 'toiletries', 'essentials']),
  "imageObjectPath": zod.string().nullable(),
  "color": zod.string().nullish(),
  "brand": zod.string().nullish(),
  "size": zod.string().nullish(),
  "season": zod.string().nullish(),
  "occasion": zod.string().nullish(),
  "purchasePrice": zod.string().nullish(),
  "purchaseDate": zod.string().nullish(),
  "notes": zod.string().nullish(),
  "isFavorite": zod.boolean().optional(),
  "timesWorn": zod.number().optional(),
  "createdAt": zod.coerce.date()
})).optional(),
  "createdAt": zod.coerce.date()
})


/**
 * @summary Remove an item from an outfit
 */
export const RemoveItemFromOutfitParams = zod.object({
  "id": zod.coerce.number(),
  "itemId": zod.coerce.number()
})

export const RemoveItemFromOutfitResponse = zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "notes": zod.string().nullish(),
  "itemIds": zod.array(zod.number()),
  "items": zod.array(zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "category": zod.enum(['outfits', 'beauty', 'toiletries', 'essentials']),
  "imageObjectPath": zod.string().nullable(),
  "color": zod.string().nullish(),
  "brand": zod.string().nullish(),
  "size": zod.string().nullish(),
  "season": zod.string().nullish(),
  "occasion": zod.string().nullish(),
  "purchasePrice": zod.string().nullish(),
  "purchaseDate": zod.string().nullish(),
  "notes": zod.string().nullish(),
  "isFavorite": zod.boolean().optional(),
  "timesWorn": zod.number().optional(),
  "createdAt": zod.coerce.date()
})).optional(),
  "createdAt": zod.coerce.date()
})


/**
 * @summary Rename a saved outfit
 */
export const RenameOutfitParams = zod.object({
  "id": zod.coerce.number()
})




export const RenameOutfitBody = zod.object({
  "name": zod.string().min(1)
})

export const RenameOutfitResponse = zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "notes": zod.string().nullish(),
  "itemIds": zod.array(zod.number()),
  "items": zod.array(zod.object({
  "id": zod.number(),
  "name": zod.string(),
  "category": zod.enum(['outfits', 'beauty', 'toiletries', 'essentials']),
  "imageObjectPath": zod.string().nullable(),
  "color": zod.string().nullish(),
  "brand": zod.string().nullish(),
  "size": zod.string().nullish(),
  "season": zod.string().nullish(),
  "occasion": zod.string().nullish(),
  "purchasePrice": zod.string().nullish(),
  "purchaseDate": zod.string().nullish(),
  "notes": zod.string().nullish(),
  "isFavorite": zod.boolean().optional(),
  "timesWorn": zod.number().optional(),
  "createdAt": zod.coerce.date()
})).optional(),
  "createdAt": zod.coerce.date()
})


/**
 * @summary Delete a saved outfit
 */
export const DeleteOutfitParams = zod.object({
  "id": zod.coerce.number()
})

export const DeleteOutfitResponse = zod.void()


/**
 * @summary Request a presigned URL for file upload
 */





export const RequestUploadUrlBody = zod.object({
  "name": zod.string().min(1),
  "size": zod.number().min(1),
  "contentType": zod.string().min(1)
})






export const RequestUploadUrlResponse = zod.object({
  "uploadURL": zod.string().url(),
  "objectPath": zod.string(),
  "metadata": zod.object({
  "name": zod.string().min(1),
  "size": zod.number().min(1),
  "contentType": zod.string().min(1)
}).optional()
})


/**
 * @summary Serve a public asset
 */
export const GetPublicObjectParams = zod.object({
  "filePath": zod.coerce.string()
})

export const GetPublicObjectResponse = zod.unknown()


/**
 * @summary Serve an uploaded object
 */
export const GetStorageObjectParams = zod.object({
  "objectPath": zod.coerce.string()
})

export const GetStorageObjectResponse = zod.unknown()


